...
import { renderToString } from "https://esm.sh/preact-render-to-string@5.2.0?deps=preact@10.8.2";
...
export async function html(options: Options): Promise<Response> {
  ...
  return new Response(
    `<!DOCTYPE html>` + renderToString(<Html {...context} />),
    {
        ...
        {(colorScheme === "auto" || colorScheme === "dark") && (
          <style
            dangerouslySetInnerHTML={{
              __html: `.dark{color-scheme:dark}.dark ::-moz-selection{background:#444}.dark ::selection{background:#444}`,
            }}
          />
        )}
        {colorScheme === "auto" && (
          <script
            dangerouslySetInnerHTML={{
              __html: `(()=>{let v=localStorage.getItem("color-scheme"),a=window.matchMedia("(prefers-color-scheme: dark)").matches,cl=document.documentElement.classList,setColorScheme=v=>(!v||v==="auto"?a:v==="dark")?cl.add("dark"):cl.remove("dark");setColorScheme(v);window.setColorScheme=v=>{setColorScheme(v);localStorage.setItem("color-scheme",v)};})();`,
            }}
          />
        )}
        <script
          defer
          data-domain="mlava.dev"
